...
const NOTA_METAS_SIN_ACTIVIDADES = `METAS SIN ACTIVIDADES DIARIAS — si el usuario ya planifica su día a día en otra herramienta (ej. contenido en un planificador aparte, o desarrollo de proyectos), NO uses "reparto" ni "actividades": dejá "actividades": [] y expresá la meta con "fases" (tramos con su "cantidadObjetivo" y fechas, SIN reparto) o, si es una sola cantidad, con "cantidadObjetivo" en el Entregable y sin fases. La app muestra esas metas en la vista de la semana y del mes, y el usuario anota lo hecho con un botón "+1". Para una cuota que progresa por mes (ej. semana 1er mes: 3 videos; 2do mes: 4; 3er mes: 5), armá UNA Fase por tramo con semanas completas (lunes a domingo) y "cantidadObjetivo" = cuota semanal × cantidad de semanas del tramo; la app calcula sola cuánto toca por semana. Si se repite un patrón semanal fijo (ej. "Lunes: video, martes: post"), es un Entregable con "recurrencia" (aparece en el calendario los días que toca) y la cantidad va en el Entregable o sus Fases. CONTENIDO PARA REDES: usá EXACTAMENTE estas unidades — "videos", "posts", "carruseles", "historias" — porque el Planificador de Contenido de la app las reconoce: al publicar una pieza suma solo +1 a la meta y toma la cuota semanal como mezcla sugerida de cada semana. El detalle de cada semana (ideas, días, guiones) NO se arma acá: se planifica semana a semana en el Planificador de Contenido.

ACTIVIDADES DE DEDICACIÓN DIARIA (ej. "Desarrollo de proyectos", "Estudiar"): si lo que importa es reservar el tiempo todos los días y el usuario decide cada día qué hace, NO inventes actividades ni cantidades: usá UN Entregable con "recurrencia" (frecuencia "dias_especificos" y sus días) y SIN cantidadObjetivo, con "fases": [] y "actividades": []. El usuario después anota en cada día qué va a hacer o qué hizo y a qué proyecto se dedicó, y la app suma el tiempo por proyecto. Si además hay un resultado mensual (ej. "1 proyecto terminado por mes"), va en OTRO Entregable con una Fase por mes.`;
