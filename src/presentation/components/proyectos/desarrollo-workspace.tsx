...
import {
  generarClaudeMd as generarClaudeMdPuro,
  generarPromptEstacion as generarPromptEstacionPuro,
} from "../../../domain/prompts/generar-prompt-desarrollo";
import { generarMarkdownContextoEjecucion } from "./desarrollo/generar-contexto-ejecucion";
...
export const DesarrolloWorkspace: React.FC<DesarrolloWorkspaceProps> = ({
  ...
  const generarClaudeMd = (): string => generarClaudeMdPuro(proyecto);
  ...
  const generarPromptEstacion = (
    ...
    generarPromptEstacionPuro(proyecto, tareas, historia, estacion, execution);
  ...
  const descargarContextoCompleto = async () => {
    try {
      const md = await generarMarkdownContextoEjecucion(
        proyecto,
        ticketExecutions
      );
